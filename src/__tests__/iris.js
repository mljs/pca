...
describe('iris dataset', function () {
  ...
  it('standard deviation', function () {
    let result = [1.7084, 0.9560, 0.3831, 0.1439];
    let standardDeviations = pca.getStandardDeviations();
    expect(standardDeviations).toBeDeepCloseTo(result, 4);
  });
  it('explained variance', function () {
    let result = [0.7296, 0.2285, 0.03669, 0.00518];
    let explainedVariance = pca.getExplainedVariance();
    expect(explainedVariance).toBeDeepCloseTo(result, 4);
  });
  it('cumulative variance', function () {
    let result = [0.7296, 0.9581, 0.9948, 1];
    let cumulativeVariance = pca.getCumulativeVariance();
    expect(cumulativeVariance).toBeDeepCloseTo(result, 4);
  });
  it('prediction', function () {
    var pred = pca.predict(iris.slice(0, 2));
    expect(pred).toBeDeepCloseTo([
